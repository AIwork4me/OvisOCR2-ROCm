...
Hard rules (no silent fallback — D5):
  * ``windows-hip + vllm`` is rejected (Qwen3-Next GDN has no HIP-SDK serving
    path) — we never silently fall back to ``smoke``.
  * ``server_url`` is rejected for the in-process ``vllm`` backend — vLLM is
    served IN-PROCESS, so a server URL is configuration noise that would be
    silently ignored; surface it as an error instead.
  * ``trust_remote_code`` defaults True because OvisOCR2 ships a custom
    ``qwen3_5`` architecture that cannot load without it; the trade-off is
    documented in rocmdoc.yaml + REPRO.yaml and the weights revision is recorded
    in provenance (ADR-0003 trust bar).
"""
...
ENV_LOCAL = Path(__file__).resolve().parents[2] / "adapter" / "setup" / ".env.local"
...
def resolve(cli: dict) -> Config:
    ...
    cfg = Config(
        ...
        resume=_pick(cli.get("resume"), "OVISOCR2_RESUME", False, _bool),
...
def validate(cfg: Config) -> None:
    ...
    # D5: vLLM is served IN-PROCESS — a server_url is unused configuration that
    # must not be silently ignored. Reject it explicitly.
    if cfg.backend == "vllm" and cfg.server_url:
        raise SystemExit(
            "server_url is not supported by the in-process vllm backend (vLLM runs embedded; "
            "no external server is contacted). Remove --server-url, or use an api-server backend.")
    if cfg.batch_size < 1:
